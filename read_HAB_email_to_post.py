...
password_mgr.add_password(None,top_level_url,'flbahr',password)
...
for jj in ll:
    ...
    #    pdb.set_trace()
    mid=letterids[jj]
    # below is set up for testing particular messages
    #mid='96134'
    ...
    m=mybody.split()
    # we can use e-mail access above so the following lines aren't needed.
    #m=x.split()
    #fis=[i for i, y in enumerate(m) if (y=="From:")]
    #whom=m[fis[0]+1]+' '+m[fis[0]+2]
    #
    ...
    #ld=np.arange(1,len(dis)) # first date is part of the mail header and not the body
...
#        HAB_station[stations[siteindex[0]]].at[counts[siteindex[0]],'date']=mydates[nd]
    ...
    #
    # dis tells us when we are in the second part of the message
...
#                    pdb.set_trace()
    ...
    # need code to see if we want to stop this
    [wdindex,wtemp]=find_word_val(my,m,'Water Temperature')
    wtemp=find_wtemp(wdindex,wtemp,m)
...
f.close()
#nd=np.arange(0,len(HAB_station['Stearns']['date']))
#mytimes=[]
#for i in nd:
#    mytimes.append(datetime.date.fromordinal(int(HAB_station['Stearns']['date'][i]))) 
